...
pub fn handle_write(req: Request) -> Response {
  ...
  case json.parse(body, write_decoder()) {
    Error(_) ->
      wisp.bad_request("Expected {\"project_path\":\"...\",\"files\":[...]}")
...
pub fn handle_migrate(req: Request) -> Response {
  ...
  case json.parse(body, migrate_decoder()) {
    Error(_) ->
      wisp.bad_request("Expected {\"sql\":\"...\",\"project_path\":\"...\"}")
    Ok(#(sql, project_path)) -> {
          ...
          wisp.json_response(
            json.to_string(
              json.object([
                ...
                #("hint", json.string("psql $DATABASE_URL -f " <> full_path)),
              ]),
            ),
            200,
          )
...
fn node_to_gleam(node: ApiNode) -> String {
  case node.kind {
    ...
    "Enum" -> {
      let vs =
        node.fields |> list.map(fn(f) { "  " <> f.name }) |> string.join("\n")
      "pub type " <> node.name <> " {\n" <> vs <> "\n}"
    }
    "Union" -> {
      let vs =
...
fn node_to_sdl(node: ApiNode) -> String {
  case node.kind {
    ...
    "Union" -> {
      let members =
        node.fields |> list.map(fn(f) { f.field_type }) |> string.join(" | ")
...
fn gen_sql(nodes: List(ApiNode)) -> String {
  ...
  let tables =
    obj |> list.map(fn(n) { node_to_sql(n, names) }) |> string.join("\n\n")
...
fn node_to_sql(node: ApiNode, obj_names: List(String)) -> String {
  ...
  let field_cols =
    ...
    |> list.map(fn(f) {
      "  " <> to_snake_case(f.name) <> " " <> sql_col_type(f, obj_names)
    })
